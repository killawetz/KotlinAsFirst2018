...
data class Circle(val center: Point, val radius: Double) {
     ...
     */
    fun distance(other: Circle): Double = if (center.distance(other.center) > radius + other.radius) center.distance(other.center) - radius - other.radius
    else 0.0
...
fun findNearestCirclePair(vararg circles: Circle): Pair<Circle, Circle> {
    ...
    for (i in 0 until circles.size) {
        for (j in i + 1 until circles.size) {
            if (circles[i].center.distance(circles[j].center) > circles[i].radius + circles[j].radius) {
                if (circles[i].center.distance(circles[j].center) - circles[i].radius - circles[j].radius < min) {
                    min = circles[i].center.distance(circles[j].center) - circles[i].radius - circles[j].radius
                    first = circles[i]
                    second = circles[j]
                }
            } else continue
